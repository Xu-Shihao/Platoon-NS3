...
static void Pause(TCPStream* stream, std::queue<veh_Nodes>* vehicles,std::queue<veh_Nodes>* vehicles2, std::map<int, double>* numb_pos_pair,std::map<int, double>* numb_pos_pair_2)
{
	...
	if (Simulator::Now ().GetSeconds () == 0.0)
	{
		...
		//cout <<"In schedule 178" << endl;
		// Schedule the merging event which will be the WSMP packet sent by c8 to the 
		// leader at T=100
		plat -> schedule_merge_event(178, 100, 1);
...
int main(int argc, char* argv[])
{
 ...
 // Acceptor will not start listening for connections until simulation configuration is complete in NS3
		...
		int numVehicleNodes= 14;
		int numVehicleNodes1= 7;// first platoon length 
		int numVehicleNodes2= 7;// second platoon length
...
//cout<< "before node loop"<<endl;
		...
		for ( int i = numVehicleNodes1 ; i < numVehicleNodes2+numVehicleNodes1 ; i++)
		{
			...
			veh2.veh_number = 0;
			veh2.in_use = 0;
			veh2.veh_device = DynamicCast<WaveNetDevice> (devices.Get (i));
			veh2.node_mac_address = veh2.veh_device->GetAddress ();
			//Ptr<WaveNetDevice>  sender = DynamicCast<WaveNetDevice> (devices.Get (i));
			//Simulator::Schedule (Seconds (0.0), &WaveNetDevice::StartSch, sender, schInfo);
			//Simulator::Schedule (Seconds (0.0), &WaveNetDevice::StartSch, veh.veh_device, schInfo);
			std::cout << veh2.node_mac_address << std::endl;
			vehicles2.push(veh2);
			//open channel switching
		   ...
		   if (acceptor->start() == 0)
			...
			while (1)
			{
				...
				//Config::Connect ("/NodeList/0/DeviceList/0/$ns3::WifiNetDevice/Phy/State/Tx",MakeBoundCallback (&TxSent, stream));
				 AnimationInterface anim("Platoon2.xml");
