...
PlatoonGroup::PlatoonGroup(int psize, std::queue<veh_Nodes>* vehicles, std::map<int, double>* mymap, ns3::Time first,int psize2, std::queue<veh_Nodes>* vehicles2, std::map<int, double>* mymap2, ns3::Time first2, TCPStream* stream) :  streamer(stream), ack (0)  /// What the constructor has to do is to create the vehicles
{
	...
	//second platoon
	//cout << "insude constructor 2" << endl;
	...
	{
		std::cout<<platoon_members[i].veh_node<<std::endl;
...
void PlatoonGroup::schedule_merge_event(double Sch, uint32_t packetSize,uint32_t numPack)
{
	...
	Ptr<WaveNetDevice>  sender = platoon_members[7].veh_device;
  	Ptr<WaveNetDevice>  receiver = platoon_members[0].veh_device;
...
// ( Merge_req and ACK ) and based on that an action will be required to be taken. For merge there has to 
...
bool PlatoonGroup::ReceivePacket (Ptr<NetDevice> dev, Ptr<const Packet> pkt, uint16_t mode, const Address &sender)
{
   ...
   if (receiver == platoon_members[0].veh_device && ack == 0)
   {
	PlatoonGroup::SendAcknowledgement (sender, receiver); //reply 1
        std::cout<< "Merge Starts" << std::endl;
	Ptr<WaveNetDevice>  sender1 = platoon_members[0].veh_device;
	Ptr<WaveNetDevice>  receiver1 = platoon_members[7].veh_device;
  	const static uint16_t WSMP_PROT_NUMBER = 0x88DC;
  	//Mac48Address bssWildcard = Mac48Address::GetBroadcast ();
	...
	// Schedule a send event 1 seconds from now where the message conveyed is merge_accept
   ...
   if (receiver == platoon_members[7].veh_device && ack==1 &&pkt->GetSize () != 80)
   {
	PlatoonGroup::SendAcknowledgement (sender, receiver);
	ack++;
   }
   if (recvAdd != platoon_members[7].node_mac_address && recvAdd != platoon_members[0].node_mac_address )
   {
	PlatoonGroup::SendAcknowledgement (sender, receiver);   		
	ack++;
  ...
  /* // platoon follower Sends Change_pl to all of its followers
   if (receiver == platoon_members[9].veh_device && ack == 1 && pkt->GetSize () != 80)
   {

	   Ptr<WaveNetDevice>  sender2 = platoon_members[9].veh_device;
	   Ptr<WaveNetDevice>  receiver2 = platoon_members[10].veh_device;
	   const static uint16_t WSMP_PROT_NUMBER = 0x88DC;
	  //Mac48Address bssWildcard = Mac48Address::GetBroadcast ();
    ...
    if (receiver == platoon_members[0].veh_device && ack == 2)
   {
      		std::cout<<"Inside ack=2" <<std::endl;
   		Ptr<WaveNetDevice>  sender3 = platoon_members[7].veh_device;
   		const static uint16_t WSMP_PROT_NUMBER = 0x88DC;
