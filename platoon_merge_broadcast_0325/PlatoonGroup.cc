...
void PlatoonGroup::schedule_merge_event(double Sch, uint32_t packetSize,uint32_t numPack)
{
	...
	TypeId tid = TypeId::LookupByName ("ns3::UdpSocketFactory");
	Ptr<Socket> source = Socket::CreateSocket (platoon_members[0].veh_node, tid);
	InetSocketAddress remote = InetSocketAddress (platoon_members[7].veh_ip_address, 80);
...
void PlatoonGroup::ReceivePacket (TCPStream* ss2, struct veh_Nodes* v,std::vector<veh_Nodes>* platoon_members, Ptr<Socket> socket)
{
	...
	{
	    std::cout<< " reveiver is: "<<v->veh_number<<" at " <<  Simulator::Now ().GetSeconds () <<std::endl;
            char buf[20];
            for (int i=0; i<20 ; i++)
            {
                buf[i] = 0;
